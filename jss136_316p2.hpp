...
void printArr(int dist[], int n){
    printf("To Vertex   Distance from Source\n");
    for (int i = 0; i < n; ++i)
        printf("%d \t\t %d\n", i, dist[i]);
}
...
Graph* openFile(){
    ...
    string filename;
    int num_vertex, num_edges, bgn, dest, weight;
    ...
    Graph* graph = createGraph(num_vertex);
    ...
    while(!fileObj.eof()){
        ...
        addEdge(graph, bgn, dest, weight);
    }
    fileObj.close();

    cout << "An adjacency list has been created to represent your directed graph\n";

    return graph;
}

int getSource(Graph* graph){
    int src;

    cout << "Now pick a vertex to be the source. The index values start at 1 and end at " << graph->V+1 << ":";
    cin >> src;

    return (src-1);
}
#endif // JSS136_316P2_HPP_INCLUDED
